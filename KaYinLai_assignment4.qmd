...
for (i in 1:length(results)) {
  ...
  summary[row_idx, col_idx] <- results[[i]]
}
...
By comparing each model, first, we can see that the 10-point AGHQ model with intercept = -2 (sim8) has a bias nearest to 0 which means it get the smallest differences between an estimate and the true value. Second, the glm model with intercept = 2 (sim5) has a lowest variance that its sampled data is close to where the model predicted it would be. Last, comparing the scaled rmse, the 10-point AGHQ model with intercept = -2 contains the smallest, it suggests that it is more able to predict the target variable accurately. Overall, the 10-point AGHQ model with intercept = -2 shows the best performance.
