...
**BMB**: why did you set `eval:false` ??

## load the required libraries

**BMB**: technically these are "packages"
...
library(dotwhisker)
library(performance)
library(mlbench) ## what for?
library(see)
library(emmeans)
library(effects)
library(pracma) ## what for?
```
...
The response variable is using units. For all predictor variable, a range of 0-10 would consider as a reasonable threshold for a small change in that variable.

**BMB**: all variables are *percentages* ...

**Fit a linear regression model**

To fit a linear regression model, the four predictor variables included are 'complains', 'learning' 'raises', 'advance'.

**BMB**: how/why did you choose these?
...
**BMB**: I don't see where these examples cover "several different levels of violation". It looks like you ran it only for a single example?
